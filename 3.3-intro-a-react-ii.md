...
Para comenzar, vamos a crear un nuevo módulo Javascript para definir el componente. Recuerda que es buena práctica ordenar nuestros componentes en una carpeta para ellos, en `src/components` Crearemos un archivo `RandomCat.js` dentro de esta carpeta `components` recién creada. Tendremos que importar React de su módulo y exportar el componente, así que partiremos de la siguiente estructura:
...
function RandomCat() {
  return (
   // AQUÍ ESCRIBIREMOS NUESTRO COMPORTAMIENTO
  )
}
